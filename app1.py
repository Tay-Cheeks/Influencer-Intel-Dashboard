...
REGION_CPM_BENCHMARK_USD = {
    "US": (10.0, 25.0),
    "UK": (8.0, 20.0),
    "EU": (8.0, 20.0),
    "South Africa": (3.0, 9.0),
    "Other": (5.0, 15.0),
}
...
def _value_verdict(quoted_fee: float, recommended_fee: float) -> str:
    if recommended_fee <= 0:
        return "N/A"
    ratio = quoted_fee / recommended_fee
    if ratio > 1.15:
        return "Overpriced"
    if ratio < 0.85:
        return "Great value"
...
with st.sidebar:
    ...
    st.divider()
    creator_region = st.selectbox(
        "Creator Region",
        ["US", "UK", "EU", "South Africa", "Other"],
        index=3,
    )

    client_currency = st.selectbox("Client Currency", ["USD", "GBP", "EUR", "ZAR"], index=3)
    creator_currency = st.selectbox("Creator Currency", ["USD", "GBP", "EUR", "ZAR"], index=3)

    st.divider()
    client_cost = st.number_input("Quoted Fee (Client)", min_value=0.0, step=100.0)
    agency_margin = st.slider("Agency Margin (%)", 0, 50, 20)

    st.divider()
    expected_views_method = st.radio(
        "Expected Views Basis",
        ["Median views", "Average views"],
        index=0,
        help="Expected views defaults to median for stability; average can be inflated by viral outliers.",
    )

    benchmark_low_usd, benchmark_high_usd = REGION_CPM_BENCHMARK_USD.get(creator_region, (5.0, 15.0))
    benchmark_mid_usd = (benchmark_low_usd + benchmark_high_usd) / 2
    default_target_cpm = _convert_currency(benchmark_mid_usd, "USD", client_currency)
    target_cpm = st.number_input(
        "Client Target CPM",
        min_value=0.0,
        step=1.0,
        value=float(round(default_target_cpm, 2)),
        help="Editable default is based on a placeholder region benchmark. Update mapping in code if needed.",
    )
...
if run and creator_url:
    try:
        ...
        st.error("Could not fetch channel data. Please check the URL/handle and try again.")
        st.stop()

    if not channel or "uploads_playlist_id" not in channel:
        st.error("Could not fetch channel data. Please check the URL/handle and try again.")
        st.stop()
    ...
    try:
        ...
        st.error("Could not fetch recent videos for this channel. Please try again.")
        st.stop()

    if not videos:
        st.error("No recent videos were found for this channel.")
        st.stop()
    ...
    if not report or "median_views" not in report or "mean_views" not in report:
        st.error("Not enough data to calculate performance metrics for this channel.")
        st.stop()

    try:
        exchange_rate_client_to_creator = _convert_currency(1.0, client_currency, creator_currency)
    except ValueError:
        st.error("Unsupported currency selection. Please select from the available currencies.")
        st.stop()

    expected_views = report["median_views"] if expected_views_method == "Median views" else report["mean_views"]
    if expected_views <= 0:
        st.error("Expected views must be greater than 0. Please try another creator/channel.")
        st.stop()
    if target_cpm <= 0:
        st.error("Client target CPM must be greater than 0.")
        st.stop()
    if client_cost < 0:
        st.error("Quoted fee must be 0 or higher.")
        st.stop()

    recommended_fee_client = (target_cpm / 1000.0) * expected_views
    recommended_fee_creator = _convert_currency(recommended_fee_client, client_currency, creator_currency)
